package huixinquan.DataStructure.Sorter;
...
public class Sorter {
    ...
    public void selectSort(Object[] r,int low,int high){
        for(int k=low;k<high-1;k++){
            ...
            for(int i=min+1;i<=high;i++){
                if(strategy.compare(r[i],r[min])<0)//找到当前的发现最小值来作为参考
                    min=i;
            }
            if(k!=min){
                Object temp=r[k];
    ...
    public void bottomUpSort(Object[] r,int low,int high){
        ...
        while(t<high-low+1){
            ...
            if(i+s<high)
                merge(r,i+1,i+s,high);
        }
    }

    public static void main(String[] args) {
        Sorter sorter=new Sorter(new huixinquan.strategy.DefaultStrategy());
